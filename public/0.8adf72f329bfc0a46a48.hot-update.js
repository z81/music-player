	...
	var Element = (function (_React$Component) {
		...
		_createClass(Element, [{
			...
			value: function prev() {
				...
				this.setState(function () {
					return {
						track: _this2.state.track > 1 ? _this2.state.track - 1 : _this2.state.music.length - 1
					};
				});
			}
		}, {
			...
			value: function next() {
				...
				this.setState(function () {
					return {
						track: _this3.state.track < _this3.state.music.length - 1 ? _this3.state.track + 1 : 0
					};
				});
			}
		}, {
			...
			value: function changeVolume(e) {
				...
				var volume = (100 - (e.pageY - el.offsetTop)) / 100;

				this.setState(function () {
					_this4.state.audio.volume = volume;
					return {
						volume: volume
					};
				});
			...
			value: function render() {
				...
				var music = _react2['default'].createElement('div', null);
				...
				if (this.state.music !== []) {
					music = _react2['default'].createElement(
						...
						this.state.music.map(function (t, i) {
							var selectedClass = i === _this5.state.track ? "current_selected" : "";
							...
							return _react2['default'].createElement(_materialUi.ListItem, { key: i, className: selectedClass, primaryText: t.title, secondaryText: t.artist[0], secondaryTextLines: 1, onClick: selectTrack });
						})
					);
				}
						...
						_react2['default'].createElement(
							...
							_react2['default'].createElement(
								...
								_react2['default'].createElement(
									'div',
									{ id: 'current-time' },
									Math.round(this.state.audio.currentTime / 60),
									':',
									Math.round(this.state.audio.currentTime % 60)
								)
							)
